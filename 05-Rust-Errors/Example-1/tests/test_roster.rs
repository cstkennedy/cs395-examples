...
#[rstest]
fn test_non_default_constructor<'a>(default_courses: (Roster<'a>, Roster<'a>)) {
...
#[rstest]
fn test_set_course_num<'a>(default_courses: (Roster<'a>, Roster<'a>)) {
...
#[rstest]
fn test_set_enroll_limit<'a>(default_courses: (Roster<'a>, Roster<'a>)) {
...
fn test_enroll<'a>(
    ...
    assert_that!(cs725.enroll(john.clone()), is(true));
    assert_that!(cs725.enroll(tom.clone()), is(true));
    assert_that!(cs725.enroll(jay.clone()), is(true));
    assert_that!(cs725.enroll(oscar.clone()), is(false)); // should fail (limit of 3);
    ...
    assert_that!(cs725.enroll(oscar), is(true)); // should succeed (limit of 4);
    ...
    assert_that!(cs725.enroll(tom), is(false)); // should fail
