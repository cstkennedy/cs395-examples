...
use enroll_students::parser::{self, PlainTextParser, StudentParser, RosterParser};
...
fn main() -> eyre::Result<()> {
    ...
    let all_rosters: Vec<EmptyRoster> = match args.roster_format {
        FileFormat::Text => RosterParser::read_from_text(&args.roster_filename)?,
        FileFormat::Toml => RosterParser::read_from_toml(&args.roster_filename)?,
    };
