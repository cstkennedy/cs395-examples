...
use enroll_students::parser::{self, PlainTextParser, RosterParser, StudentParser, TomlParser};
...
fn main() -> eyre::Result<()> {
    ...
    let all_students = <StudentParser as PlainTextParser>::read_items(&args.student_filename)?;

    /*
    let all_rosters: Vec<EmptyRoster> = match args.roster_format {
        FileFormat::Text => RosterParser::read_items(&args.roster_filename)?,
        FileFormat::Toml => RosterParser::read_items(&args.roster_filename)?,
    };
    */

    let all_rosters: Vec<EmptyRoster> = match args.roster_format {
        FileFormat::Text => <RosterParser as PlainTextParser>::read_items(&args.roster_filename)?,
        FileFormat::Toml => <RosterParser as TomlParser>::read_items(&args.roster_filename)?,
    };
