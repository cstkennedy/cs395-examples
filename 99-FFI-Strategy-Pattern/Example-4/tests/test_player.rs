...
pub fn test_next_move(a_player: Player) {
    ...
    assert_that!(
        player.next_move(),
        is(equal_to(Ok(Position::try_from(5 as usize).unwrap())))
    );
    assert_that!(
        player.next_move(),
        is(equal_to(Ok(Position::try_from(1 as usize).unwrap())))
    );
    assert_that!(
        player.next_move(),
        is(equal_to(Ok(Position::try_from(3 as usize).unwrap())))
    );
}
