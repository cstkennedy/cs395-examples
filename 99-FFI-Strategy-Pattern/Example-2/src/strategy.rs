...
use crate::error::{PositionError, StrategyCreationError, StrategyError};
...
pub struct PredefinedMoves {
    my_moves: Vec<Position>,
    move_idx: usize,
}

impl PredefinedMoves {
    pub fn from_iterable_discarding<I>(
        raw_moves: I,
    ) -> Result<PredefinedMoves, StrategyCreationError>
    where
        I: IntoIterator<Item = usize>,
    {
        let some_moves: Vec<usize> = raw_moves.into_iter().collect();

        let validated_moves: Vec<Position> = some_moves
            .iter()
            .copied()
            .flat_map(Position::try_from)
            .collect();

        if validated_moves.is_empty() {
            return Err(StrategyCreationError::NoValidPositions(some_moves.into()));
        }
    ...
    pub fn from_iterable_nondiscarding<I>(
        raw_moves: I,
    ) -> Result<PredefinedMoves, StrategyCreationError>
    where
        I: IntoIterator<Item = usize>,
    {
        ...
        if validated_moves.len() != some_moves.len() {
            return Err(PositionError::BatchValueError(some_moves.into()).into());
        }
