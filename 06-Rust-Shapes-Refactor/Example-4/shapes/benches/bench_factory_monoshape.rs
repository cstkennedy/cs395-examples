...
use shapes::factory::{CreationFactory, FactoryDirectory, MonoFactory};
...
fn bench_is_known_valid(name: &str) {
    let _ = MonoFactory::is_known(black_box(name));
}
...
fn bench_number_known() {
    let _ = MonoFactory::number_known();
}

#[divan::bench(min_time = 1)]
fn bench_list_known() {
    let _ = MonoFactory::list_known();
}
...
fn bench_create(name_and_vals: (&str, &[f64])) {
    ...
    let _ = MonoFactory::create_default(black_box(&name));
}

#[divan::bench(min_time = 1, args = SHAPE_TUPLES)]
fn bench_create_with(name_and_vals: (&str, &[f64])) {
    let (name, dims) = name_and_vals;
    let _ = MonoFactory::create_with(black_box(&name), black_box(&dims));
}

#[divan::bench(min_time = 1)]
fn bench_create_with_invalid() {
    let _ = MonoFactory::create_with(black_box("Triangle"), black_box(&[1.0, 3.0]));
}
