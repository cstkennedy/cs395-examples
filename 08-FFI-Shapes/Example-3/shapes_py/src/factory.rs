...
impl ShapeFactory {
    ...
    pub fn create_with(name: &str, dims: Vec<f64>) -> PyResult<ShapeWrapper> {
        match name {
            "Circle" => Ok(Circle::try_from(&dims[..]).unwrap().into()),
            "Square" => Ok(Square::try_from(&dims[..]).unwrap().into()),
            "Triangle" => Ok(Triangle::try_from(&dims[..]).unwrap().into()),
            "Equilateral Triangle" => Ok(EquilateralTriangle::try_from(&dims[..]).unwrap().into()),
            "Right Triangle" => Ok(RightTriangle::try_from(&dims[..]).unwrap().into()),
            _ => Err(pyo3::exceptions::PyKeyError::new_err(format!(
                "'{name}' is not known"
            ))),
        }
