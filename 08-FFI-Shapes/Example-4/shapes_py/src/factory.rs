...
impl ShapeFactory {
    ...
    pub fn create(name: &str) -> PyResult<ShapeWrapper> {
        match name {
            ...
            _ => Err(ShapeCreationError::UnknownShapeError(name.into()).into()),
        }
    }
    ...
    pub fn create_with(name: &str, dims: Vec<f64>) -> Result<ShapeWrapper, ShapeCreationError> {
        match name {
            "Circle" => Ok(Circle::try_from(&dims[..])?.into()),
            "Square" => Ok(Square::try_from(&dims[..])?.into()),
            "Triangle" => Ok(Triangle::try_from(&dims[..])?.into()),
            "Equilateral Triangle" => Ok(EquilateralTriangle::try_from(&dims[..])?.into()),
            "Right Triangle" => Ok(RightTriangle::try_from(&dims[..])?.into()),
            _ => Err(ShapeCreationError::UnknownShapeError(name.into())),
        }
    }
